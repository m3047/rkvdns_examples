...
Command line:
    ...
    clients_fanout.py <window> [<server>] [<rkvd-domain> [<dns-server>]] {+count} {+trend}
    
Lists the number of requests and completion status for clients.
    ...
    rkvd-domain  The FQDN which "fans out" to multiple RKVDNS domains using PTR
                 records.
...
RKVDNS Fanout Proof of Concept
------------------------------

This copypasta version of clients.py utilizes the totalizer.fanout.BaseName
implementation of total() to aggregate from multiple federated RKVDNS / Redis
data sources.
"""
...
def lart(msg=None, help='clients window [server] [rkvdns-domain [dns-server]]'):
    if msg:
